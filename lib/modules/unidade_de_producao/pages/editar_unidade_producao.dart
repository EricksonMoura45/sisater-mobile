...
class EditarUnidadeProducao extends StatefulWidget {
  final UnidadedeProducaoList unidade;
  
  const EditarUnidadeProducao({super.key, required this.unidade});

  @override
  State<EditarUnidadeProducao> createState() => _EditarUnidadeProducaoState();
}

class _EditarUnidadeProducaoState extends State<EditarUnidadeProducao> {
  ...
  Status _statusEdicao = Status.CONCLUIDO;
  ...
  Future<void> _carregarDados() async {
    try {
      ...
      // Preencher dados da unidade existente
      ...
      setState(() {
        _statusCarregaDados = Status.ERRO;
      });
    }
  }
  
  void _preencherDadosExistentes() {
    final unidade = widget.unidade.productionUnitNormal;
    if (unidade != null) {
      _denominacaoController.text = unidade.name ?? '';
      _detalhamentoAcessoController.text = unidade.accessDetails ?? '';
      _areaDocumentalController.text = unidade.legalArea?.toString() ?? '';
      _areaRealController.text = unidade.realArea?.toString() ?? '';
      _areaPreservacaoController.text = unidade.preservationArea?.toString() ?? '';
      _areaCulturaPermanenteController.text = unidade.permanentCultivationArea?.toString() ?? '';
      _areaCulturaTemporariaController.text = unidade.temporaryCultivationArea?.toString() ?? '';
      _areaPastagemController.text = unidade.pastureArea?.toString() ?? '';
      _areaReservaLegalController.text = unidade.legalReserveArea?.toString() ?? '';
      _areaOutrosController.text = unidade.otherArea?.toString() ?? '';
      
      _logradouroController.text = unidade.street ?? '';
      _numeroController.text = unidade.number ?? '';
      _complementoController.text = unidade.complement ?? '';
      _bairroController.text = unidade.neighborhood ?? '';
      _cepController.text = unidade.postalCode ?? '';
      
      _possuiCar = unidade.hasCar == 1;
      _possuiSeguro = unidade.hasSafe == 1;
      
      // Selecionar município
      if (unidade.cityCode != null) {
        _municipioSelecionado = _listaMunicipios.firstWhere(
          (m) => m.code == unidade.cityCode,
          orElse: () => Municipio(),
        );
        if (_municipioSelecionado?.code != null) {
  ...
  Future<void> _carregarComunidades() async {
    ...
    try {
      ...
      if (cityCode != null) {
        ...
        // Selecionar comunidade se existir
        final unidade = widget.unidade.productionUnitNormal;
        if (unidade?.communityId != null) {
          _comunidadeSelecionada = comunidades.firstWhere(
            (c) => c.id == unidade!.communityId,
            orElse: () => Comunidade(),
          );
        }
      }
  ...
  Future<void> _carregarTransicaoAgroecologica() async {
    try {
      ...
      // Selecionar transição se existir
      final unidade = widget.unidade.productionUnitNormal;
      if (unidade?.agroecologicalTransitionId != null) {
        _transicaoAgroecologicaSelecionada = transicoes.firstWhere(
          (t) => t.id == unidade!.agroecologicalTransitionId,
  ...
  Future<void> _carregarDominio() async {
    try {
      ...
      // Selecionar domínio se existir
      final unidade = widget.unidade.productionUnitNormal;
      if (unidade?.domainId != null) {
        _dominioSelecionado = dominios.firstWhere(
          (d) => d.id == unidade!.domainId,
          orElse: () => Dominio(),
        );
      }
    } catch (e) {
  ...
  Future<void> _carregarEnergiaEletrica() async {
    try {
      ...
      // Selecionar energia se existir
      final unidade = widget.unidade.productionUnitNormal;
      if (unidade?.electricPowerId != null) {
        _energiaEletricaSelecionada = energias.firstWhere(
          (e) => e.id == unidade!.electricPowerId,
  ...
  Future<void> _carregarAbastecimentoAgua() async {
    try {
      ...
      // Selecionar abastecimento se existir
      final unidade = widget.unidade.productionUnitNormal;
      if (unidade?.waterSupplyId != null) {
        _abastecimentoAguaSelecionado = abastecimentos.firstWhere(
          (a) => a.id == unidade!.waterSupplyId,
  ...
  Future<void> _atualizarUnidadeProducao() async {
    ...
    try {
      setState(() {
        _statusEdicao = Status.AGUARDANDO;
      });
      ...
      // Enviar para API
      await _repository.putUnidadeDeProducao(unidadeProducao, widget.unidade.id!);
      
      setState(() {
        _statusEdicao = Status.CONCLUIDO;
      });
      
      _mostrarMensagem('Unidade de produção atualizada com sucesso!');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: formAppBar(context, 'Editar Unidade de Produção', false, ''),
      body: Observer(
        builder: (_) {
  ...
  Widget botaoAtualizar(BuildContext context) {
    return Observer(builder: (_) {
      if (_statusEdicao == Status.AGUARDANDO) {
