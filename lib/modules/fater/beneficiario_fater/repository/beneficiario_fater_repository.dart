...
class BeneficiarioFaterRepository {
  Dio dio;
  BeneficiarioFaterRepository(this.dio);

  bool? listaFaterbool = false;
  bool? carregaMunicipios = false;
  bool? listaAter = false;
  AppStore appStore = AppStore();
  String? cityCodeFater;
  
  Future<List<BeneficiarioFaterList>> listaBeneficiariosFater(int page) async {
    var response = await dio.get(
      '/ater',
      queryParameters: {
        'type': 1, //Para trazer todos os registros de organizacoess Ater
        'pageSize': 20,
        'page': page,
        'sort': '-created_at'
      }
    ...
    List<BeneficiarioFaterList> beneficiariosFater = [];

    (response.data as List).map((a) => beneficiariosFater.add(BeneficiarioFaterList.fromJson(a))).toList();

    if(response.statusCode == 200){
      listaFaterbool = true;
      //print(response.statusCode);
    }
    return beneficiariosFater; 
  }
  ...
  Future<List<Proater>> listaProater() async {
    var response = await dio.get(
      '/proater',
      queryParameters: {
        'sort': 'name',
    ...
    (response.data as List).map((a) => listaProater.add(Proater.fromJson(a))).toList();

    return listaProater;
  }

  Future<List<UnidadeMedida>> listaUnidadeMedida() async {
    var response = await dio.get(
      '/unit-of-measure',
      queryParameters: {
        'sort': 'name',
    ...
    (response.data as List).map((a) => listaUnidadeMedida.add(UnidadeMedida.fromJson(a))).toList();
  ...
  Future<bool> deleteFater(int id) async {
