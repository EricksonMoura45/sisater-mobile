...
class BeneficiarioChatPage extends StatefulWidget {
  final ChatCard? chat;
  
  const BeneficiarioChatPage({super.key, this.chat});

  @override
  State<BeneficiarioChatPage> createState() => _BeneficiarioChatPageState();
}

class _BeneficiarioChatPageState extends State<BeneficiarioChatPage> {
  
  final BeneficiarioController beneficiarioController = Modular.get();
  ...
  void didChangeDependencies() {
    ...
    // Recebe o chat selecionado via arguments ou widget.chat
    chat = widget.chat ?? (Modular.args.data as ChatCard?);
    if (chat != null) {
      beneficiarioController.setChatSelecionado(chat);
      ...
      if (autenticacaoController.usuarioDados == null) {
        autenticacaoController.carregaUsuario().then((_) {
          beneficiarioController.getMensagens(chat!.id!);
        });
      } else {
        beneficiarioController.getMensagens(chat!.id!);
      }
  ...
  void _enviarMensagem() async {
    ...
    final sucesso = await beneficiarioController.enviarMensagem(chat!.id!, texto);
    if (sucesso) {
              ...
              Expanded(
                child: Observer(
                  builder: (_) {
                    ...
                    switch (beneficiarioController.statusCarregaMensagens) {
                              ...
                              ElevatedButton(
                                onPressed: () {
                                  ...
                                  if (chatId != null && chatId > 0) {
                                    beneficiarioController.getMensagens(chatId);
                                  }
                                },
                      ...
                      case Status.CONCLUIDO:
                        final mensagens = beneficiarioController.mensagens;
  ...
  Widget _buildInputArea() {
    final statusChat = chat?.status?.toString().toLowerCase();
    final isFechado = statusChat == 'fechado' || statusChat == 'closed';
